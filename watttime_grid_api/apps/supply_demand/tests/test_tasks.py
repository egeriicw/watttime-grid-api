...
class TestInsertGeneration(TestCase):
    ...
    def setUp(self):
        self.dp_dict = {'freq': '5m', 'ba_name': 'MISO', 'market': 'RT5M',
              ...
              'timestamp': pytz.utc.localize(datetime.utcnow())}
        self.dp_forecast_dict = {'freq': '1hr', 'ba_name': 'MISO', 'market': 'DAHR',
              'fuel_name': 'coal', 'gen_MW': 58906,
              'timestamp': pytz.utc.localize(datetime.utcnow())}
    ...
    def test_insert_forecast(self):
        dp_id = tasks.insert_generation(self.dp_forecast_dict)
        gen = Generation.objects.get(fuel__name=self.dp_forecast_dict['fuel_name'],
                                     mix__ba__abbrev=self.dp_forecast_dict['ba_name'])
        self.assertEqual(dp_id, gen.mix.id)

    def test_insert_series_rtm(self):
        # start without series
        self.assertEqual(DataSeries.objects.count(), 0)

        # insert data, now there's a series
        tasks.insert_generation(self.dp_dict)
        ...
        # insert again, no new data added
        tasks.insert_generation(self.dp_dict)
        self.assertEqual(DataSeries.objects.count(), 1)
        self.assertEqual(DataSeries.objects.get(series_type=DataSeries.CURRENT).datapoints.count(), 1)

    def test_insert_series_forecast(self):
        # start without series
        self.assertEqual(DataSeries.objects.count(), 0)

        # insert data, still no series
        tasks.insert_generation(self.dp_forecast_dict)
        self.assertEqual(DataSeries.objects.count(), 0)
